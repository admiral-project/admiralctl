...
func TestOperationsList(t *testing.T) {
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Write([]byte(`[{"id":"op1","action":"provision","status":"succeeded","created_at":"2023-01-01"}]`))
	}))
	defer ts.Close()
	...
	if !bytes.Contains(buf.Bytes(), []byte("op1")) {
		t.Errorf("expected output to contain op1, got %q", buf.String())
	}
}
